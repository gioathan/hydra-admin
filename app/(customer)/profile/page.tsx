...
function MenuIcon({ type }: { type: IconType }) {
  if (type === "edit") return (
    <svg className="w-5 h-5" style={{ color: "#041635" }} fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
  ...
  return (
    <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24" style={{ color: "#ba1a1a" }}>
...
function MenuRow({ href, label, iconType, destructive, onClick }: {
    ...
    <>
      <div className="flex items-center gap-3">
        <div
          ...
          style={{ background: destructive ? "#ffdad6" : "rgba(27,43,75,0.12)" }}
        >
          <MenuIcon type={iconType} />
        </div>
        <span
          className="text-sm font-medium"
          style={{ color: destructive ? "#ba1a1a" : "#041635", fontFamily: "var(--font-sans)" }}
        >
          {label}
        </span>
      </div>
      {!destructive && (
        <svg className="w-5 h-5" style={{ color: "#75777f" }} fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
  ...
  const cls = "flex items-center justify-between px-4 py-3.5 hover:bg-[#f5f3f6] active:bg-[#f5f3f6] transition-colors w-full";
...
function DesktopCard({ href, title, subtitle, iconType }: {
  ...
  const style = { background: "#ffffff", borderColor: "#e4e2dd" };
  const inner = (
    <>
      <div
      ...
      <div className="flex-1">
        <p className="font-semibold" style={{ color: "#041635", fontFamily: "var(--font-sans)" }}>{title}</p>
        <p className="text-sm mt-0.5" style={{ color: "#75777f" }}>{subtitle}</p>
      </div>
      <svg className="w-5 h-5 flex-shrink-0" style={{ color: "#75777f" }} fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
      </svg>
    </>
  );
...
function SummaryRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between">
      <span className="text-sm" style={{ color: "#75777f" }}>{label}</span>
      <span className="text-sm font-medium" style={{ color: "#041635" }}>{value}</span>
    </div>
  );
}
    ...
    <>
      ...
      <div className="lg:hidden flex flex-col min-h-full" style={{ background: "#fbf8fc" }}>
        <div
          ...
          style={{ background: "#ffffff", borderBottom: "1px solid rgba(197,198,207,0.4)" }}
        >
          <div
            className="w-20 h-20 rounded-full flex items-center justify-center text-3xl font-bold text-white mb-2"
            style={{ background: "#9c440f", fontFamily: "var(--font-serif)" }}
          >
            {getInitial(name)}
          </div>
          <p className="text-lg font-semibold" style={{ color: "#041635", fontFamily: "var(--font-sans)" }}>{name}</p>
          <p className="text-sm" style={{ color: "#44474e" }}>{customer?.email ?? customer?.phone}</p>
        </div>

        <div className="flex-1 px-5 py-5 flex flex-col gap-5">
          <div>
            <p className="text-xs font-bold tracking-widest uppercase mb-2.5 ml-1" style={{ color: "#44474e", fontFamily: "var(--font-sans)" }}>Account</p>
            <div className="rounded-xl overflow-hidden" style={{ background: "#ffffff", border: "1px solid rgba(197,198,207,0.3)", boxShadow: "0 1px 4px rgba(4,22,53,0.04)" }}>
              <MenuRow href="/profile/edit" label="Edit Profile" iconType="edit" />
              <div style={{ height: 1, background: "rgba(197,198,207,0.4)", marginLeft: 64 }} />
              <MenuRow href="/profile/password" label="Change Password" iconType="lock" />
            </div>
          </div>

          <div>
            <p className="text-xs font-bold tracking-widest uppercase mb-2.5 ml-1" style={{ color: "#44474e", fontFamily: "var(--font-sans)" }}>Bookings</p>
            <div className="rounded-xl overflow-hidden" style={{ background: "#ffffff", border: "1px solid rgba(197,198,207,0.3)", boxShadow: "0 1px 4px rgba(4,22,53,0.04)" }}>
          ...
          <div>
            <p className="text-xs font-bold tracking-widest uppercase mb-2.5 ml-1" style={{ color: "#44474e", fontFamily: "var(--font-sans)" }}>Support</p>
            <div className="rounded-xl overflow-hidden" style={{ background: "#ffffff", border: "1px solid rgba(197,198,207,0.3)", boxShadow: "0 1px 4px rgba(4,22,53,0.04)" }}>
          ...
          <div>
            <p className="text-xs font-bold tracking-widest uppercase mb-2.5 ml-1" style={{ color: "#44474e", fontFamily: "var(--font-sans)" }}>Legal</p>
            <div className="rounded-xl overflow-hidden" style={{ background: "#ffffff", border: "1px solid rgba(197,198,207,0.3)", boxShadow: "0 1px 4px rgba(4,22,53,0.04)" }}>
          ...
          <p className="text-[10px] text-center tracking-widest uppercase mt-2" style={{ color: "#75777f", fontFamily: "var(--font-sans)" }}>Hydra v1.0</p>
        </div>
      </div>
        ...
        <div style={{ background: "#041635", padding: "72px 0 56px" }}>
          <div className="max-w-[1440px] mx-auto px-20 flex items-center gap-8">
            <div
              ...
              style={{ background: "#9c440f", fontSize: 40, fontFamily: "var(--font-serif)" }}
            >
              {getInitial(name)}
            </div>
            <div>
        ...
        <div className="max-w-[1440px] mx-auto px-20 py-16 w-full">
          <div className="grid grid-cols-12 gap-8">
            ...
            <div className="col-span-8 flex flex-col gap-10">
              ...
              <div>
                <h2 className="text-xs font-bold tracking-widest uppercase mb-5" style={{ color: "#75777f", fontFamily: "var(--font-sans)" }}>Bookings</h2>
                <div className="flex flex-col gap-3">
              ...
              <div>
                <h2 className="text-xs font-bold tracking-widest uppercase mb-5" style={{ color: "#75777f", fontFamily: "var(--font-sans)" }}>Support</h2>
                <div className="flex flex-col gap-3">
                  <DesktopCard href="mailto:test@email.com" title="Contact Us" subtitle="Get in touch with our support team" iconType="mail" />
                </div>
              </div>
              <div>
                <h2 className="text-xs font-bold tracking-widest uppercase mb-5" style={{ color: "#75777f", fontFamily: "var(--font-sans)" }}>Legal</h2>
                <div className="flex flex-col gap-3">
            ...
            <div className="col-span-4">
              <div className="sticky top-32 p-8 rounded-xl border flex flex-col gap-5" style={{ background: "#f5f3ee", borderColor: "#c5c6cf" }}>
                <h3 className="text-xs font-bold uppercase tracking-widest" style={{ color: "#75777f", fontFamily: "var(--font-sans)" }}>Contact Info</h3>
                <div className="flex flex-col gap-3">
                ...
                <button
                  ...
                  style={{ color: "#ba1a1a", border: "1px solid #ba1a1a", background: "transparent" }}
                >
                  Sign Out
                </button>
              </div>
